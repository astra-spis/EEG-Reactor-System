Shader "E2G2/EEGModel"
{
    Properties
    {
        [Header(EEGModel)][Space]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                {
                    // 無効なデータの場合は既存のデータを保持
                    return tex2Dlod(_Buffer, float4(i.uv, 0.0, 0.0));
                }

                // 同期した書き込み関数を使用
                float4 compressedData;
                compressedData = WriteSynchronizedFrameData(currentBands, currentFrameIndex, _Buffer, i.uv);
